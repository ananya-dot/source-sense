"""
This file contains the workflow definition for the Supabase metadata extraction application.

Note:
- The run method is overriden from the base class for demonstration purposes.
"""
...
@workflow.defn
class SupabaseMetadataExtractionWorkflow(BaseSQLMetadataExtractionWorkflow):
    @observability(logger=logger, metrics=metrics, traces=traces)
    @workflow.run
    async def run(self, workflow_config: Dict[str, Any]):
        """
        Run the Supabase metadata extraction workflow.

        :param workflow_args: The workflow arguments.
        """
        activities_instance = SupabaseMetadataExtractionActivities()
    ...
    @staticmethod
    def get_activities(
        activities: SupabaseMetadataExtractionActivities,
    ) -> List[Callable[..., Any]]:
